...
$conditions = (object) array(
	...
	{
		$weightings = Array(
			new Weighting(0, 0, 0, 0),
			new Weighting(1, 1, -1, 0.5),//beach
			new Weighting(-0.5, -1, 1, 0),//cinema
			new Weighting(0, 0, 0, 0),//stadium
			new Weighting(0, 0, 0, 0) //cafe
		);
		...
		for ($i = 0; $i < $activitiesLength; $i++)
		{
			...
			$scoreFactors = array();
			$scoreFactors[] = $weightings[$activities[$i]->type]->condition * getConditionNumber($conditions->weather->condition);
			$scoreFactors[] = $weightings[$activities[$i]->type]->temperature * $conditions->weather->temperature;
			$scoreFactors[] = $weightings[$activities[$i]->type]->precipitation * $conditions->weather->precipitation;
			$scoreFactors[] = $activities[$i]->distance_adjusted;
			$activities[$i]->score = array_sum($scoreFactors) / count($scoreFactors);	
		}
	...
	class Weighting
	{
		...
		function __construct($weather, $temp, $rain, $_time)
		{
