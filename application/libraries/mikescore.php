...
class MikeScore {
	...
	public function calculateScores($activities, $conditions)
	{
		$weightings = Array(
		...
		for ($i = 0; $i < $activitiesLength; $i++)
		{
			...
			$activities[$i]->score += $weightings[$activities[$i]->type]->condition * $this->getConditionNumber($conditions->weather->condition);
			$activities[$i]->score += $weightings[$activities[$i]->type]->temperature * $this->standardise($conditions->weather->temp, 30, 0);
			$activities[$i]->score += $weightings[$activities[$i]->type]->precipitation * $this->standardise($conditions->weather->precipitation, 50, 0);
			
			$activities[$i]->score += $weightings[$activities[$i]->type]->distance * $this->standardise($activities[$i]->distance, 0, 30000);
			$activities[$i]->score += $this->timeAppropriateness($weightings[$activities[$i]->type]->timefeel, $conditions->_time);
		}
		
		return $activities;
	}
	
	private function timeAppropriateness($timefeel, $time)
	{
		$score = 0;
	...
	private function standardise($number, $upper, $lower)
	{
	...
	private function getConditionNumber($condition)
	{
		if ($condition == "sun")
		{
			return 1;
